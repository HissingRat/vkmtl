...
`ResourceTablePressureDescriptor` 和
`Device.planResourceTablePressure(...)` 用来在分配前总结大型 resource table 的压力。
...
`PipelineArtifactCachePlanDescriptor` 表示。`Device.planPipelineArtifactCache(...)`
...
`StabilityRunDescriptor` 用来描述 opt-in long-run checks，不会强行塞进默认测试。它可以规划
