...
不需要窗口的任务使用 `HeadlessContext`。它持有 backend device 和 queue 状态，但不会创建
surface、swapchain、drawable 或 presentation queue。它返回的 `Device` 和 `Queue` 与窗口
context 使用同一套 resource、shader、pipeline 和 command API。
...
`HeadlessContext.Options` 还接受 `adapter_selection` 和
`debug_backend_override`。Current-drawable render pass 和 present 不可用；绑定 texture view
的 offscreen render pass 仍然可用。所有资源和已提交工作必须在
`HeadlessContext.deinit()` 前结束。
...
`PresentationDescriptor.format` 是应用的请求。当前允许的 SDR 请求是 `.automatic`、
`.bgra8_unorm_srgb` 和 `.bgra8_unorm`。`.automatic` 会确定性地先选 sRGB BGRA8，再选
linear BGRA8；两个显式请求都必须精确满足，否则初始化或重建会返回
`UnsupportedPresentationFormat`。`Swapchain.presentationDescriptor()` 继续返回原始请求，
`Swapchain.selectedFormat()` 则返回具体选择且不会是 `.automatic`：
...
用于 current drawable 的 pipeline 必须精确匹配这个 selected format；不匹配会在 native
pipeline bind 或 draw 之前返回 `PresentationFormatMismatch`。成功的非零 resize 会保留请求并
发布当前 concrete selection；只有重新查询或 recreate native presentation state 的 resize 才会
从保留的请求重新解析。健康的 Vulkan same-request resize 是不查询 surface 的 no-op。复用依赖
format 的 pipeline 前应再次查询 `selectedFormat()`。Offscreen attachment 与
`HeadlessContext` 不参与 presentation selection。

`Swapchain.presentationDescriptor().extent` 同样是 requested extent，
`Swapchain.extent()` 才是当前 actual native drawable extent。Vulkan surface 的
`currentExtent` 或 clamp 可能让两者不同。健康状态下的 zero-size resize 会保留上次成功的
request、actual extent 和 selected format。

健康 Vulkan runtime 对相同 requested extent 的 resize 会走 cheap no-op。Present/acquire
出现 `SUBOPTIMAL` 或 `OUT_OF_DATE` 后会标记 recovery required，所以下一次即使请求相同也会
rebuild。requested extent 改变时会重新查询 native state；如果解析出的 native configuration
没有变化，可以只记录请求而不 recreate。Metal 只有在 replacement depth allocation 成功后才
发布新的 drawable extent。

Vulkan 上，每次非零 resize 和 `Swapchain.clear(...)` 都要求 window context 创建的 backend
command buffer 已全部 commit；否则会在改变 native presentation state 前返回
`InvalidCommandBufferState`。Clear 使用独立 internal command pool，不会 reset caller command
pool。如果 native
swapchain recreate 或依赖资源 rebuild 已开始但随后失败，presentation runtime 会永久 lost：
本次调用返回原始错误，之后的 resize（包括 zero）、clear 或新建 command buffer 都返回
`SurfaceLost`。出现这个终态后应重建 `WindowContext`。

Resolver 只选择和校验 SDR attachment format。vkmtl 不执行 HDR conversion、exposure、tone
mapping、gamma correction、EOTF/OETF 或 gamut conversion；应用负责写入 selected
attachment 的数值。
...
Runtime `Device` 是资源创建入口。`WindowContext.device()` 和
`HeadlessContext.device()` 返回各自 owner 的 device view。
...
timeline fence 由 `DeviceFeatures.timeline_fences` gate，并映射为 Vulkan timeline semaphore
或 Metal shared event。`Device.makeEvent(...)`
会创建带 `signal(...)`、`wait(...)`、`reset()` 和 `isSignaled()` 的 `Event`。Shared
event 是 native Metal shared event，仍由 `DeviceFeatures.shared_events` gate；它不承诺
external handle import/export。
...
`CommandBuffer.commitWithSynchronization(...)`。Native timeline/shared-event operation 会进入
backend submission；binary fence 和 ordinary event 保留精确的 host-side fallback。两条路径都会
校验 object lifetime、device/backend identity 和 monotonic value。
...
Period 52 让这些 maintenance plan 可执行：把
`AccelerationStructureMaintenanceResources` 传给
`CommandBuffer.encodeAccelerationStructureMaintenance(...)`。Update/refit 使用 built、
`allow_update` 的 source 和 acceleration-structure scratch buffer；compact 使用 built source
和单独 destination，不接受 scratch，而且 source 必须在 build 时设置
`AccelerationStructureBuildFlags.allow_compaction`。Metal 和 Vulkan 都会提交原生 update/refit/compact-copy
命令。Build/update scratch size 是原生查询；post-build compacted-size query 仍不支持。
Source 引用的 build-input buffer 和 TLAS instance-source AS 必须保持存活，直到所有复用
它们的 update/refit submission 完成。
...
shader requirements。它目前是 planning/native-availability contract，不是 executable support：
Metal 没有直接等价语义，而 Vulkan 普通 compute/render binding 还不能绑定 AS，
所以两个 backend 的 usable `ray_query` 都是 false。
...
对应的 usable feature report gate，
...
presentation command 保留。它要求 caller output 是覆盖完整 texture 的 single-sample
`bgra8_unorm` 2D view，具有 shader-write 与 copy-source usage，extent 与 presentation 完全一致。
两个 backend 都先向这个对象 dispatch，再把原始字节复制到 selected linear 或 sRGB BGRA8
drawable 后呈现。这个 copy 不执行 sampling、filtering、transfer-function conversion、tone
mapping 或 gamut conversion。新代码应使用 texture command 和显式 composition pass。仓库示例
默认保留 canonical 路径；设置 `VKMTL_RT_LEGACY_DRAWABLE=1` 才会选择有限帧兼容验证路径。
Legacy command 自带隐式 present；在同一个 command buffer 上再显式调用
`presentDrawable(...)` 会返回 `InvalidCommandBufferState`。
这个 legacy dispatch/present 组合操作只允许 graphics queue；compute/transfer queue 会在 native
work 前返回 `InvalidQueueCapability`。Metal 会在 compute dispatch 前获取并校验 drawable，并预先
分配 sRGB raw-copy staging buffer；linear 路径不分配 staging。
...
Period34 的 Vulkan procedural sphere path。Period 52 又补齐了 Metal 原生 AABB BLAS build，
并支持一个 TLAS 引用多个不同 BLAS source；非默认 transform/mask/custom-index/SBT-offset
metadata 仍只是 planning contract。
...
并接入 procedural `ray_traced_scene` 验收路径。Metal schema-2 artifact 没有 linked
intersection function 和 driver-bound table，因此 custom intersection/function table 仍明确不支持；
planning mapping 不会冒充 native table。
...
用于设置 borrowed label、可选 lifecycle callback，并为后续 pooling/reuse hint 留出字段。默认
...
允许假定 reentrant command-buffer use。如果 backend commit 失败，这个 one-shot command buffer
也会被终态消费：backend object 会 deinit，active-command/query-resolve borrow 会释放，work
serial 会退休，lifecycle status 变为 `failed`。Vulkan 在销毁 temporary command resource 前会
等待任何已经收到该 work 的 queue。当前 command buffer 在 `commit()` 后仍然是 one-shot；pooled 或 reusable
...
会选择 physical compute 或 transfer queue。Metal 创建独立 command queue；Vulkan 查询 queue
family，并在存在时使用 dedicated family，否则按 descriptor 的 fallback policy 处理。
...
`InvalidQueueOwnershipState`。Metal 把 native queue ordering 与 tracked ownership 组合起来；Vulkan
resource 对已选择的 work family 使用 concurrent native sharing，同时由 vkmtl 保持 exclusive logical
ownership。Public API 不暴露 raw queue-family release/acquire control。

`CommandBuffer.presentDrawableWithDescriptor(...)` 提供 capability-gated presentation timing。
`.immediate` 是默认值；`.at_monotonic_time` 和 `.after_minimum_duration` 需要非零纳秒值以及对应
device feature。不支持时返回 typed error，只有 `allow_immediate_fallback` 明确允许才会回落到
immediate。Metal 原生映射两个 timed mode；Vulkan 当前明确不支持 timed mode。
`presentDrawable()` 仍是 immediate convenience call。
