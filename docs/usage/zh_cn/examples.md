...
| Voxel world | `zig build run-voxel-world` | 窗口；通过 profile/frame-limit/autopilot 环境变量执行有界有限帧运行 | 有界 visible-face chunk renderer，覆盖 atlas material、camera/culling、streaming、指标和 `voxel_world_pressure_test=ok`。 |
...
| Ray-traced scene | `zig build run-ray-traced-scene` | 支持时显示窗口；`VKMTL_RT_FRAME_LIMIT` 可限定帧数 | Native RT 写入 scene-linear `rgba16_float`，再由共享 ACES-fitted fullscreen pass 呈现并打印 backend-specific `driver_pixels=visible_...` marker；否则给出可执行 unsupported 诊断。 |
...
`examples/voxel_world` 是已经完成的、有明确上限的 Minecraft-like renderer 压力测试。
它使用确定性的 `16 x 64 x 16` chunk、带跨 chunk 邻居检查的 visible-face CPU meshing、
每 chunk vertex/index buffer、生成式 atlas、reflection 派生 layout、camera uniform、
depth/back-face culling、CPU chunk culling，以及有界 rebuild/upload；不导入后端私有 API。

`smoke`、`default` 和 `stress` profile 的半径分别为 1、4、8，resident chunk 上限分别为
9、81、289。通过 `VKMTL_VOXEL_PROFILE` 选择 profile，默认值为 `default`。
`VKMTL_VOXEL_FRAME_LIMIT=N` 精确限制帧数；`VKMTL_VOXEL_AUTOPILOT=1` 会移动 camera
并定期触发 rebuild。
...
操作方式：`W/A/S/D` 水平移动，`Q/E` 垂直移动，鼠标或方向键控制视角，Shift 加速，
`R` 重建 camera 所在 chunk，Escape 退出。退出报告包含 resident、visible/culled、pending、
draw/vertex/index、rebuild/retire/upload/allocation、CPU mesh/encode/commit 时间和 frame
p50/p95/max。

有限帧成功会打印 `voxel_world_pressure_test=ok`。Apple M4 Pro 上启用 Metal API
Validation 的 smoke/default/stress 运行均达到 9/81/289 resident 上限并成功退出。Vulkan
artifact 和 forced build 已通过，但这里不声称已经执行物理 Vulkan voxel workload。
...
`rgba16_float` scene-linear texture。第二个 public render command 采样它，执行固定 exposure
`1.0` 和共享 ACES-fitted curve，再把 display-linear color 返回给
`bgra8_unorm_srgb`；attachment 负责唯一一次 sRGB encode。旧 drawable dispatch command
仍为兼容保留。RT dispatch 与 fullscreen consumer 使用两个 command buffer，因为当前每个
command buffer 只拥有一个 native encoding segment。

Metal API Validation 已物理执行这个新路径 3 帧。上面的历史 Vulkan 物理证据仍能证明 native RT
backend，但早于 Period 55 display transform。新的 Vulkan color-managed path 已有 unit 与
forced-build evidence，仍需在 Vulkan RT 机器上物理复跑。
