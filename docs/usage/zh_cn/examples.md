...
case 仍会创建一个小 GLFW surface 来初始化 backend，但确定性检查完成后会自行结束。
...
| Bindless textures | `zig build run-bindless-textures` | 自动退出 probe | 打印 `bindless texture table ok: ...` 或 `bindless textures unsupported: ...`。 |
| Multi-window | `zig build run-multi-window` | 双窗口 probe | 打印两个 surface record，再打印可用或预期 feature-gate 行。 |
| External texture | `zig build run-external-texture` | 自动退出 probe | 打印 `external texture wrapper ok: ...` 或明确 unsupported 行。 |
| Streaming texture | `zig build run-streaming-texture` | 自动退出 probe | 打印 residency success 或 `streaming texture unsupported: ...`。 |
| Tessellation | `zig build run-tessellation` | 自动退出 plan probe | 打印 portable/所选 native plan，或 typed unsupported 行。 |
| Mesh shader | `zig build run-mesh-shader` | 自动退出 plan probe | 打印 portable/所选 native plan，或 typed unsupported 行。 |
...
`examples/tessellation` 和 `examples/mesh_shader` 会验证 portable `vkmtl.render` planning，
并显式检查 `vkmtl.native.vulkan` 或 `vkmtl.native.metal`。在所选 backend
...
和共享 RT scene buffer 被路由到 Period35。物理 Metal 与 Vulkan RT 输出都已经 observed；
