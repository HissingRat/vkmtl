...
vkmtl 会保持普通 command path portable：resource usage tracking、fence、event、timestamp 和
occlusion query 都通过 backend-neutral runtime object 暴露。Timestamp value 可能是 logical
ordering value，也可能是 raw native GPU tick；先检查 `resultSource()`，在 calibration 尚未公开时
不要计算 duration。Occlusion 默认使用 zero/nonzero visibility，并要求 render pass 绑定对应
set；精确 count 需要 `occlusion_counting_queries`。Vulkan 原生执行显式 barrier，Metal 组合
encoder ordering 与 tracked hazard。Timeline/shared-event submit、physical queue selection、
ownership validation 和 exact occlusion counting 会在 gate 打开时执行。Pipeline statistics、
pass-boundary/device-specific counter 和 timestamp calibration 在 scalar query contract 下明确
unsupported。

## Advanced Features

Advanced features 会继续放在 feature gate 后面。Resource table、driver cache、heap、Vulkan
tessellation、mesh-only pipeline、普通 RT maintenance 和 same-device Metal import 已有可执行
路径。Sparse residency、Vulkan external import、Metal 4 allocator/pipeline/dataset object、
tensor/ML 与 multi-counter statistics 明确 unsupported。
...
resource table 可以 update、clear，并通过 render / compute encoder 绑定。Pipeline 需要声明匹配的
`resource_table_layouts`；Metal 使用 native argument buffer，Vulkan 使用已启用的
descriptor-indexing set。错误的 layout/index 会在 backend work 前返回 typed error。
...
CPU-authored 的可复用 draw / dispatch list 通过
`vkmtl.command.makeIndirectCommandBuffer(...)` 创建。它们继承当前 encoder state；Metal 在支持时
使用 native ICB，Vulkan 精确展开为 direct command。GPU-authored command mutation 仍不支持。
...
变化时会确定性失效。Render / compute pipeline 的 `driver_cache` descriptor 现在会消费并持久化
native `VkPipelineCache` / `MTLBinaryArchive` 数据；native pipeline library 仍是独立 backend work。
