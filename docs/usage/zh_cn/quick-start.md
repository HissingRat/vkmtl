...
Compute、transfer、ray tracing、resource 和绑定 texture view 的 offscreen render 可以完全跳过
窗口初始化：
...
该路径不创建 window、surface、swapchain、drawable 或 presentation queue。绑定 texture view
的 render attachment 仍然有效；current-drawable pass 和 present 操作返回 typed presentation
错误。销毁 context 前必须先销毁资源。
...
资源是显式 handle，必须在所属的 `WindowContext` 或 `HeadlessContext` 前销毁。Debug build
会追踪资源生命周期，如果 context 销毁时仍有资源未释放，会 panic。
...
真正无窗口的确定性 readback 示例见 `examples/transfer_readback` 和
`examples/compute_readback`。两者都不初始化或链接 GLFW；transfer readback 还会验证绑定
texture view 的 offscreen clear。
...
- `Device`、`Queue` 和 `Swapchain` 分别负责资源创建、command 和 presentation；
  `WindowContext` 不再转发这些操作，`HeadlessContext` 刻意不暴露 `Swapchain`。
