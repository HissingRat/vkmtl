...
| MTL-RES-006 | `MTLTexture`, `MTLTextureViewDescriptor` | Create subresource views and compatible format/swizzle reinterpretations | resource | incomplete | incomplete | `VkImageView`; current vkmtl supports ranges but not format reinterpretation/swizzle | unit |
| MTL-RES-007 | `MTLPixelFormat`, `MTLTextureType`, `MTLTextureUsage` | Cover format/type/usage combinations and query exact capabilities | resource | incomplete | incomplete | Vulkan format properties plus conversion/unsupported decisions; vkmtl format set is narrow | unit |
| MTL-RES-008 | `MTLSamplerState`, `MTLSamplerDescriptor` | Filtering, addressing, LOD, compare, anisotropy, normalized coordinates, and border color | resource | incomplete | incomplete | `VkSampler`; current fixed subset is executable, full Metal sampler semantics are not inventoried | unit |
