...
| MTL-IND-001 | `MTLIndirectCommandBuffer`, descriptor | Allocate/reset CPU-authored command slots with inherited encoder state | command | composed-exact | composed-exact | Native Metal ICB when available; exact direct-command replay otherwise and on Vulkan | gpu-smoke |
| MTL-IND-002 | `MTLIndirectRenderCommand` | CPU-encode reusable non-indexed draw commands and execute ranges | command | composed-exact | composed-exact | Native Metal ICB execution when available; Vulkan expands immutable slots to `vkCmdDraw` | gpu-smoke |
| MTL-IND-003 | `MTLIndirectComputeCommand` | CPU-encode reusable threadgroup dispatch commands and execute ranges | command | composed-exact | composed-exact | Native Metal compute ICB when available; Vulkan expands immutable slots to `vkCmdDispatch` | unit |
