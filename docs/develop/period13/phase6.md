...
- For the current capability-gated slice, register two external windows through
  `SurfaceCollection` and report native multi-surface availability.
...
- Until native multi-swapchain lowering is enabled, it may exit after printing a
  clear feature-gate message.
