...
Status: Period 56 complete, 2026-07-15. Vulkan RT physical reruns remain
...
new shared-display Vulkan path has unit/forced-build evidence; its physical
RT-machine rerun remains pending and is not inferred from the earlier visible
Vulkan RT evidence.
...
| RT-03 | Triangle and AABB BLAS geometry input | `ray_tracing` | `native-exact` | `native-exact` | Physical Metal headless evidence builds both forms. Metal AS allocation takes the maximum of triangle/AABB and ordinary/update native sizes, while command validation uses the selected plan's exact scratch size. For the current Vulkan single-geometry sizing templates, allocation likewise takes the component maximum across triangle/AABB geometry and all four update/compaction combinations because native build-size queries are not monotonic across flags; the first Period 56 Windows reruns exposed the earlier combined-flags upper-bound assumption before driver submission. Descriptor-exact sizing for arbitrary multi-geometry arrays is not established by this fix and remains a separate follow-up. Vulkan procedural scene evidence already exercises AABB input; fresh Period 56 physical reruns remain pending. |
...
legacy formats are recorded for Period 56. Both Vulkan RT-machine runs remain
follow-up evidence. The exactly-once gap-routing file is empty because all 111
